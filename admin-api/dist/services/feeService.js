"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.feeService = exports.FeeService = void 0;
const fee_model_1 = require("../models/fee.model");
const user_model_1 = require("../models/user.model");
const Transaction_1 = require("../models/Transaction");
const Asset_1 = require("../models/Asset");
const node_cron_1 = __importDefault(require("node-cron"));
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
// 每月1号凌晨执行
const FEE_SCHEDULE = '0 0 1 * *';
// 基础托管费
const BASE_FEE = 100;
class FeeService {
    constructor() {
        this.startFeeScheduler();
    }
    static getInstance() {
        if (!FeeService.instance) {
            FeeService.instance = new FeeService();
        }
        return FeeService.instance;
    }
    startFeeScheduler() {
        node_cron_1.default.schedule(FEE_SCHEDULE, async () => {
            try {
                await this.processMonthlyFees();
            }
            catch (error) {
                console.error('Error processing monthly fees:', error);
            }
        });
    }
    async processMonthlyFees() {
        const users = await user_model_1.User.find({});
        for (const user of users) {
            try {
                const userId = user._id.toString();
                // 获取用户所有资产
                const assets = await Asset_1.Asset.find({ userId });
                if (assets.length === 0)
                    continue;
                // 计算总资产
                const totalBalance = assets.reduce((sum, asset) => sum + asset.balance, 0);
                // 计算托管费（新用户首月有30美元折扣）
                const isNewUser = await this.isNewUser(userId);
                const feeAmount = isNewUser ? Math.max(0, BASE_FEE - 30) : BASE_FEE;
                // 检查用户余额是否足够
                if (totalBalance < feeAmount) {
                    console.log(`Insufficient balance for user ${userId}`);
                    continue;
                }
                // 创建托管费交易记录
                await Transaction_1.Transaction.create({
                    userId,
                    type: 'FEE',
                    amount: feeAmount,
                    chain: 'BSC', // 默认从BSC链扣除
                    status: 'PENDING',
                    txHash: `FEE-${Date.now()}-${userId}`
                });
                // 更新用户资产
                const mainAsset = assets.find(asset => {
                    const assetWithChain = asset;
                    return assetWithChain.chain === 'BSC';
                });
                if (mainAsset) {
                    mainAsset.balance -= feeAmount;
                    await mainAsset.save();
                }
            }
            catch (error) {
                console.error(`Error processing fee for user ${user._id}:`, error);
            }
        }
    }
    async isNewUser(userId) {
        const thirtyDaysAgo = new Date();
        thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
        const recentFees = await fee_model_1.Fee.find({
            userId: userId.toString(),
            type: 'monthly',
            createdAt: { $gte: thirtyDaysAgo }
        });
        return recentFees.length === 0;
    }
    // 处理充值确认
    async confirmDeposit(userId, txHash, amount, chain) {
        try {
            // 查找待处理的充值交易
            const transaction = await Transaction_1.Transaction.findOne({
                userId,
                txHash,
                type: 'DEPOSIT',
                status: 'PENDING'
            });
            if (!transaction) {
                throw new Error('Transaction not found');
            }
            // 更新交易状态
            transaction.status = 'COMPLETED';
            await transaction.save();
            // 更新用户资产
            let asset = await Asset_1.Asset.findOne({ userId, chain });
            if (!asset) {
                asset = await Asset_1.Asset.create({
                    userId,
                    chain,
                    address: '', // 需要从用户信息中获取
                    balance: amount
                });
            }
            else {
                asset.balance += amount;
                await asset.save();
            }
            return true;
        }
        catch (error) {
            console.error('Error confirming deposit:', error);
            throw error;
        }
    }
    async calculateFee(user, asset, amount) {
        // 这里实现费用计算逻辑
        return amount * 0.01; // 示例：1% 的费用
    }
    async createFee(userId, amount, type) {
        const user = await user_model_1.User.findById(userId);
        if (!user) {
            throw new Error('User not found');
        }
        const userAsset = await Asset_1.Asset.findOne({ userId });
        if (!userAsset) {
            throw new Error('User asset not found');
        }
        // 确保 userAsset 有 chain 属性
        const assetWithChain = userAsset;
        if (!assetWithChain.chain) {
            throw new Error('Asset chain information is missing');
        }
        const fee = new fee_model_1.Fee({
            userId,
            amount,
            type,
            status: 'completed',
            chain: assetWithChain.chain
        });
        return await fee.save();
    }
    async processFee(feeId) {
        const fee = await fee_model_1.Fee.findById(feeId);
        if (!fee) {
            throw new Error('Fee not found');
        }
        // 创建交易记录
        await Transaction_1.Transaction.create({
            userId: fee.userId,
            type: 'fee',
            amount: fee.amount,
            status: 'completed'
        });
        // 更新费用状态
        await fee_model_1.Fee.findByIdAndUpdate(feeId, { status: 'completed' });
    }
    async getMainAsset(userId) {
        const assets = await Asset_1.Asset.find({ userId });
        if (!assets || assets.length === 0) {
            return null;
        }
        // 查找 BSC 链上的资产
        const mainAsset = assets.find(asset => {
            const assetWithChain = asset;
            return assetWithChain.chain === 'BSC';
        });
        return mainAsset;
    }
}
exports.FeeService = FeeService;
exports.feeService = FeeService.getInstance();
