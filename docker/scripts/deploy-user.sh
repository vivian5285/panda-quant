...
USER_DOMAIN=www.pandatrade.space
...
# 构建并启动服务
docker-compose -f docker-compose.user.yml up -d --build || handle_error "服务启动失败"
...
if ! docker ps | grep -q "panda-quant-user-api"; then
    handle_error "user-api 服务未启动"
fi

if ! docker ps | grep -q "panda-quant-user-ui"; then
    handle_error "user-ui 服务未启动"
fi

log "部署完成！"
log "用户端访问地址: https://www.pandatrade.space"
log "API 访问地址: https://api.pandatrade.space" 