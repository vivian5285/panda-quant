...
class VerificationModel {
    ...
    async findByEmailAndCode(email, code, type) {
        try {
            return await exports.Verification.findOne({
                email,
                code,
                type,
                isUsed: false,
    ...
    async markAsUsed(id) {
        try {
            await exports.Verification.findByIdAndUpdate(id, { isUsed: true }).exec();
        }
        catch (error) {
    ...
    async deleteByEmail(email, type) {
        try {
            await exports.Verification.deleteMany({ email, type }).exec();
        }
        catch (error) {
